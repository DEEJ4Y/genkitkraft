...
// HttpToolRepository implements httptoolrepo.HttpToolRepository using SQLite.
type HttpToolRepository struct {
	db *sql.DB
}

// NewHttpToolRepository creates a new SQLite-backed HTTP tool repository.
...
func (r *HttpToolRepository) List(ctx context.Context, limit, offset int) ([]*httptool.HttpTool, error) {
	rows, err := r.db.QueryContext(ctx,
		`SELECT id, name, description, method, url, headers, body_template, input_schema, created_at, updated_at
		 FROM http_tools ORDER BY created_at DESC LIMIT ? OFFSET ?`, limit, offset)
	if err != nil {
...
func (r *HttpToolRepository) Update(ctx context.Context, t *httptool.HttpTool) error {
	...
	result, err := r.db.ExecContext(ctx,
		`UPDATE http_tools SET name = ?, description = ?, method = ?, url = ?, headers = ?, body_template = ?, input_schema = ?, updated_at = ? WHERE id = ?`,
...
func (r *HttpToolRepository) Delete(ctx context.Context, id string) error {
	result, err := r.db.ExecContext(ctx, `DELETE FROM http_tools WHERE id = ?`, id)
	if err != nil {
