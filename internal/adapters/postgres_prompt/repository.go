...
// PromptRepository implements promptrepo.PromptRepository using PostgreSQL.
type PromptRepository struct {
	db *sql.DB
}

// NewPromptRepository creates a new PostgreSQL-backed prompt repository.
...
func (r *PromptRepository) List(ctx context.Context, limit, offset int) ([]*prompt.Prompt, error) {
	rows, err := r.db.QueryContext(ctx,
		`SELECT id, name, content, created_at, updated_at
		 FROM prompts ORDER BY created_at DESC LIMIT $1 OFFSET $2`, limit, offset)
	if err != nil {
...
func (r *PromptRepository) GetByID(ctx context.Context, id string) (*prompt.Prompt, error) {
	...
	err := r.db.QueryRowContext(ctx,
		`SELECT id, name, content, created_at, updated_at
		 FROM prompts WHERE id = $1`, id).Scan(&p.ID, &p.Name, &p.Content, &p.CreatedAt, &p.UpdatedAt)
...
func (r *PromptRepository) Delete(ctx context.Context, id string) error {
	result, err := r.db.ExecContext(ctx, `DELETE FROM prompts WHERE id = $1`, id)
	if err != nil {
