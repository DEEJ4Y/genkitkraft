...
// PlaygroundRepository implements playgroundrepo.PlaygroundRepository using MySQL.
type PlaygroundRepository struct {
	db *sql.DB
}

// NewPlaygroundRepository creates a new MySQL-backed playground repository.
...
func (r *PlaygroundRepository) AppendMessageChunk(ctx context.Context, messageID string, chunk string) (int, error) {
	...
	if err := tx.QueryRowContext(ctx,
		`SELECT COALESCE(MAX(seq), 0) + 1 FROM playground_message_chunks WHERE message_id = ?`, messageID,
	).Scan(&seq); err != nil {
		return 0, apperrors.NewAppErrorf(apperrors.Internal, "computing next chunk seq: %v", err)
	}

	_, err = tx.ExecContext(ctx,
		`INSERT INTO playground_message_chunks (id, message_id, seq, content, created_at) VALUES (?, ?, ?, ?, ?)`,
		uuid.New().String(), messageID, seq, chunk, time.Now().UTC().Truncate(time.Microsecond))
	if err != nil {
		return 0, apperrors.NewAppErrorf(apperrors.Internal, "inserting message chunk: %v", err)
	}

	if _, err := tx.ExecContext(ctx,
		`UPDATE playground_messages SET content = CONCAT(content, ?) WHERE id = ?`, chunk, messageID,
	); err != nil {
...
func (r *PlaygroundRepository) GetMessageChunksSince(ctx context.Context, messageID string, sinceSeq int) ([]playground.MessageChunk, error) {
	rows, err := r.db.QueryContext(ctx,
		`SELECT seq, content FROM playground_message_chunks WHERE message_id = ? AND seq > ? ORDER BY seq ASC`,
		messageID, sinceSeq)
	if err != nil {
...
func (r *PlaygroundRepository) setStreamingMessageStatus(ctx context.Context, messageID, status string) error {
	_, err := r.db.ExecContext(ctx,
		`UPDATE playground_messages SET status = ? WHERE id = ? AND status = ?`,
...
func (r *PlaygroundRepository) GetMessage(ctx context.Context, id string) (*playground.Message, error) {
	row := r.db.QueryRowContext(ctx,
		`SELECT id, session_id, role, content, status, created_at FROM playground_messages WHERE id = ?`, id)
...
func (r *PlaygroundRepository) GetLatestMessageBySession(ctx context.Context, sessionID string) (*playground.Message, error) {
	row := r.db.QueryRowContext(ctx,
		`SELECT id, session_id, role, content, status, created_at
		 FROM playground_messages WHERE session_id = ? ORDER BY created_at DESC LIMIT 1`, sessionID)
