...
func testProviderRepository(t *testing.T, open func(string) (*sql.DB, error), dsn string) {
	t.Helper()
	...
	repo := mysqlprovider.NewProviderRepository(db)
	ctx := context.Background()

	// Clean up any stale test providers from previous interrupted runs.
	db.ExecContext(ctx, "DELETE FROM providers WHERE provider_type = ?", string(provider.OpenAI))
