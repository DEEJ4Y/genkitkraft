...
// GapRepository implements gaprepo.GapRepository using MySQL.
type GapRepository struct {
	db *sql.DB
}

// NewGapRepository creates a new MySQL-backed gap repository.
...
func (r *GapRepository) AddReference(ctx context.Context, ref *gap.Reference) error {
	...
	ref.CreatedAt = time.Now().UTC().Truncate(time.Microsecond)
