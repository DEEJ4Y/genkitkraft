...
func TestGapRepositoryMySQL(t *testing.T) {
	dsn := containers.StartMySQLDSN(t)
	testGapRepository(t, mysqldb.Open, dsn)
}

func TestGapRepositoryMariaDB(t *testing.T) {
	dsn := containers.StartMariaDBDSN(t)
	testGapRepository(t, mysqldb.Open, dsn)
}

func testGapRepository(t *testing.T, open func(string) (*sql.DB, error), dsn string) {
	t.Helper()
	...
	repo := mysqlgap.NewGapRepository(db)
...
func seedAgent(t *testing.T, db *sql.DB, ctx context.Context, providerID string) string {
	...
	t.Cleanup(func() { db.ExecContext(ctx, "DELETE FROM agents WHERE id = ?", agentID) })
	return agentID
}
...
func seedSession(t *testing.T, db *sql.DB, ctx context.Context, agentID string) string {
	...
	_, err := db.ExecContext(ctx,
		`INSERT INTO playground_sessions (id, agent_id, title, created_at, updated_at) VALUES (?, ?, ?, NOW(), NOW())`,
		sessionID, agentID, "test-session")
	if err != nil {
		t.Fatalf("inserting test session: %v", err)
	}
	t.Cleanup(func() { db.ExecContext(ctx, "DELETE FROM playground_sessions WHERE id = ?", sessionID) })
	return sessionID
}
