...
// McpServerRepository implements mcpserverrepo.McpServerRepository using PostgreSQL.
type McpServerRepository struct {
	db *sql.DB
}

// NewMcpServerRepository creates a new PostgreSQL-backed MCP server repository.
...
func (r *McpServerRepository) List(ctx context.Context, limit, offset int) ([]*mcpserver.McpServer, error) {
	rows, err := r.db.QueryContext(ctx,
		`SELECT id, name, transport, url, headers, created_at, updated_at
		 FROM mcp_servers ORDER BY created_at DESC LIMIT $1 OFFSET $2`, limit, offset)
	if err != nil {
...
func (r *McpServerRepository) Update(ctx context.Context, s *mcpserver.McpServer) error {
	...
	result, err := r.db.ExecContext(ctx,
		`UPDATE mcp_servers SET name = $1, transport = $2, url = $3, headers = $4, updated_at = $5 WHERE id = $6`,
...
func (r *McpServerRepository) Delete(ctx context.Context, id string) error {
	result, err := r.db.ExecContext(ctx, `DELETE FROM mcp_servers WHERE id = $1`, id)
	if err != nil {
