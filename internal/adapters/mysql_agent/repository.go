...
// AgentRepository implements agentrepo.AgentRepository using MySQL.
type AgentRepository struct {
	db *sql.DB
}

// NewAgentRepository creates a new MySQL-backed agent repository.
...
func (r *AgentRepository) Update(ctx context.Context, a *agent.Agent) error {
	...
	result, err := r.db.ExecContext(ctx,
		`UPDATE agents SET name = ?, provider_id = ?, model_id = ?, system_prompt_id = ?,
		 ...
		 top_k_enabled = ?, top_k = ?, max_tool_calls = ?, gap_reporting_enabled = ?, updated_at = ? WHERE id = ?`,
