...
// GapRepository implements gaprepo.GapRepository using PostgreSQL.
type GapRepository struct {
	db *sql.DB
}

// NewGapRepository creates a new PostgreSQL-backed gap repository.
...
func (r *GapRepository) GetByID(ctx context.Context, id string) (*gap.Gap, error) {
	row := r.db.QueryRowContext(ctx, `SELECT `+gapColumns+` FROM agent_gaps WHERE id = $1`, id)
	g, err := scanGap(row)
...
func (r *GapRepository) AddReference(ctx context.Context, ref *gap.Reference) error {
	...
	_, err := r.db.ExecContext(ctx,
		`INSERT INTO agent_gap_references (id, gap_id, session_id, message_id, created_at) VALUES ($1, $2, $3, $4, $5)`,
...
func (r *GapRepository) ListReferences(ctx context.Context, gapID string) ([]*gap.Reference, error) {
	rows, err := r.db.QueryContext(ctx,
		`SELECT id, gap_id, session_id, message_id, created_at
		 FROM agent_gap_references WHERE gap_id = $1 ORDER BY created_at ASC`, gapID)
	if err != nil {
