...
export function ProviderForm({
  ...
  const [baseUrl, setBaseUrl] = useState(
    provider?.baseUrl ?? typeInfo.baseUrlDefault ?? "",
  );
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    try {
      ...
      if (isEdit) {
        ...
        if (hasConfig) body.config = config;
        const { error: err } = await fetchClient.POST(
  ...
  async function handleTest() {
    ...
    setTestResult(null);
    try {
      const { data, error: err } = await fetchClient.POST(
        ...
        <Stack gap="md">
          ...
          {typeInfo.requiresApiKey && (
            <PasswordInput
              ...
              placeholder={
                isEdit ? "Leave blank to keep existing" : "Enter API key"
              }
              required={!isEdit}
            />
          )}
          ...
          {typeInfo.configFields && typeInfo.configFields.length > 0 && (
            <>
              {typeInfo.configFields.map((field) => {
                const InputComponent = field.sensitive
                  ? PasswordInput
                  : TextInput;
                return (
                  <InputComponent
                    ...
                    onChange={(e: React.ChangeEvent<HTMLInputElement>) =>
                      setConfigField(field.name, e.currentTarget.value)
                    }
                    placeholder={
                      isEdit && field.sensitive
